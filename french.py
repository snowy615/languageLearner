...
def transcribe_audio():
    audio_file = open('/Users/snowyan/PycharmProjects/languageLearner/languageLearner/input.wav', "rb")
    response = openai.Audio.transcribe(api_key = openai.api_key,model="whisper-1",file=audio_file)
    print(response.text)
...
class Chat:
    ...
    # 调用chatgpt，并计算开销
    def ask(self, prompt):
        self.conversation_list.append({"role": "user", "content": prompt+"please give limit your response to 50 words"})
...
def main():
    ...
    while count < count_limit:  # 上下文token数量是有极限的，理论上只能支持有限轮次的对话，况且，钱花光了也就不能用了。。。
        ...
        try:
            ...
            pygame.mixer.music.load("/Users/snowyan/PycharmProjects/languageLearner/languageLearner/output.mp3")
            print("load")
